...
namespace HW6 {
	class Generics {
		...
		// User should know that the items parameter must be IEnumerable, 
		// as well as that the second param needs to be the same value 
		// as the item list
		...
		// User should know that the list of items passed must be IComparable
		public static bool IsSorted<T>(T[] items) where T : IComparable {
			bool isSorted = true;
		...
		// The item array must have an IEnumerable type, and the predicate must 
		// work for the same type as the items in the array
		...
		// Items needs to be IEnumerable, predicate needs to work with same type as items 
		// in the array. Returns a List of type T.
		...
		// Second param must be a lambda function that accepts a T param and returns the 
		// same type. Third param must be a boolean predicate. Returns an array of type T.
		public static T[] TransformIf<T>(T[] items, Func<T, T> p1, Predicate<T> p2) {
